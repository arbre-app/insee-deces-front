...
export function BlockGedcomMatches() {
  ...
  const displayDate = dateGedcom => {
    ...
    const displayDatePart = datePart => {
      ...
      return intl.formatDate(jsDate, { year: 'numeric', month: hasMonth ? 'numeric' : undefined, day: hasDay ? 'numeric' : undefined });
    };
    if (dateParsed) {
        ...
        {data.candidates.slice(0, 1000).map(id => {
          ...
          return (
            <tr key={id}>
              ...
              <td><GedcomEvent event={individual.getEventBirth()} /></td>
              <td><GedcomEvent event={individual.getEventDeath()} /></td>
              <td className="align-middle">
